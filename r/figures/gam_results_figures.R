...
pkgs <- c("ggplot2", "dplyr", "sf", "ggspatial", "gratia", "cowplot", "marginaleffects")
...
# year_sm_sh <- bind_rows(sm_goa_sh, sm_ai_sh, sm_ebs_sh) |> 
#   mutate(year = as.numeric(as.character(Year_fct)))
# 
# wc_yr_sh <- ggplot(year_sm_sh %>% add_confint, aes(y = .estimate, x = year, col = mod_reg)) +
...
sm_goa_b_sh <- smooth_estimates(mod_b_goa, smooth = "s(Year_fct):Depth_fct2shallow") |> 
  mutate(area = "Bottom", mod_reg = "GOA")
sm_ai_b_sh <- smooth_estimates(mod_b_ai, smooth = "s(Year_fct):Depth_fct2shallow") |> 
  mutate(area = "Bottom", mod_reg = "AI")
sm_ebs_b_sh <- smooth_estimates(mod_b_ebs, smooth = "s(Year_fct):Depth_fct2shallow") |> 
  mutate(area = "Bottom", mod_reg = "EBS")

# year_sm_sh_b  <- bind_rows(sm_goa_b_sh, sm_ai_b_sh, sm_ebs_b_sh) |> 
#   mutate(year = as.numeric(as.character(Year_fct)))
# 
# b_yr_sh <- ggplot(year_sm_sh_b %>% add_confint, aes(y = .estimate, x = year, col = mod_reg)) +
#   geom_point(position=position_dodge(width=0.5)) +
#   geom_errorbar(aes(ymin = .lower_ci, ymax = .upper_ci, col = mod_reg), width = 0,
#                 position = position_dodge(width=0.5)) +
#   scale_color_manual(values = c("#0072B2", "#D55E00", "#009E73")) +
#   labs(x = "Year", color = "Model region") +
#   geom_hline(yintercept = 0, lty = 2, linewidth = 0.5, col = "grey30") +
#   facet_grid(area~Depth_fct2) +
#   theme_bw() +
#   scale_x_continuous(minor_breaks = seq(1995, 2020, 5), breaks = seq(1995, 2020, 5)) +
#   scale_y_continuous(minor_breaks = seq(-2, 2, 1), breaks = seq(-2, 2, 1), limits = c(-2.1, 2.1)) +
#   guides(col = guide_legend(nrow = 1, title="Region")) +
...
#         legend.position = "top")
# 
# plots_sh <- plot_grid(wc_yr_sh + theme(legend.position = "none"), 
#                    b_yr_sh + theme(legend.position = "none"), 
#                    ncol = 1, align = 'v')
...
sm_goa_b_dp <- smooth_estimates(mod_b_goa, smooth = "s(Year_fct):Depth_fct2deep") |> 
  mutate(area = "Bottom", mod_reg = "GOA")
sm_ai_b_dp <- smooth_estimates(mod_b_ai, smooth = "s(Year_fct):Depth_fct2deep") |> 
  mutate(area = "Bottom", mod_reg = "AI")
sm_ebs_b_dp <- smooth_estimates(mod_b_ebs, smooth = "s(Year_fct):Depth_fct2deep") |> 
  mutate(area = "Bottom", mod_reg = "EBS")
...
ggsave("results/plots/year_effect_figure.png", bg = "white", 
              width = 24, height = 20, units = 'cm', dpi = 300)
       
# legend <- get_plot_component(b_yr_sh, 'guide-box-top', return_all = TRUE)
# plot_grid(plot_grid(legend, plots, ncol = 1, rel_heights = c(0.1, 1)), scale = 0.95) +
#   draw_label("Partial effect", x=  0, y=0.5, vjust= 1.5, angle=90)
# 
# ggsave("results/plots/shallow_year_effect_figure.png", bg = "white", 
...
# Bottom
sm_ebs_b_sh <- smooth_estimates(mod_b_ebs, dist = 0.025,
                                smooth = "te(longitude,latitude):Depth_fct2shallow") |> 
...
EBS_plot
ggsave(file = "results/plots/ebs_map_bot_figure.png", height = 20, width = 20, units = "cm")
...
# ggplot(data = ebs_b_hy_sm, aes(HYCOM_temp, Depth, z  = .estimate)) +
#   geom_contour_filled(alpha = 0.8) +
#   labs(fill = "Partial effect") +
#   geom_contour(breaks = c(0), color = "black", size = 1.2) +
#   scale_y_reverse() + theme_bw()
# 
# ggplot(data = goa_b_hy_sm, aes(HYCOM_temp, Depth, z  = .estimate)) +
#   geom_contour_filled(alpha = 0.8) +
#   labs(fill = "Partial effect") +
#   geom_contour(breaks = c(0), color = "black", size = 1.2) +
#   scale_y_reverse() + theme_bw()
# 
# ggplot(data = ai_b_hy_sm, aes(HYCOM_temp, Depth, z  = .estimate)) +
#   geom_contour_filled(alpha = 0.8) +
#   labs(fill = "Partial effect") +
#   geom_contour(breaks = c(0), color = "black", size = 1.2) +
#   scale_y_reverse() + theme_bw()
...
# ggplot(data = ebs_wc_hy_sm, aes(HYCOM_temp, Depth, z  = .estimate)) +
#   geom_contour_filled(alpha = 0.8) +
#   labs(fill = "Partial effect") +
#   geom_contour(breaks = c(0), color = "black", size = 1.2) +
#   scale_y_reverse() + theme_bw()
# 
# ggplot(data = goa_wc_hy_sm, aes(HYCOM_temp, Depth, z  = .estimate)) +
#   geom_contour_filled(alpha = 0.8) +
#   labs(fill = "Partial effect") +
#   geom_contour(breaks = c(0), color = "black", size = 1.2) +
#   scale_y_reverse() + theme_bw()
# 
# ggplot(data = ai_wc_hy_sm, aes(HYCOM_temp, Depth, z  = .estimate)) +
#   geom_contour_filled(alpha = 0.8) +
#   labs(fill = "Partial effect") +
#   geom_contour(breaks = c(0), color = "black", size = 1.2) +
#   scale_y_reverse() + theme_bw()
...
ggsave(file = "results/plots/hycom_by_depth_effect.png", height = 20, width = 22, units = "cm")

ggplot(data = all_hy_sm, aes(HYCOM_temp, Depth, z  = .estimate)) +
  geom_contour_filled(alpha = 0.8) +
  labs(fill = "Partial effect") +
  facet_grid(factor(type, levels = c('Water Column', 'Bottom'))~region) +
  geom_contour(breaks = c(0), color = "black", size = 1) +
...
plot_smooth(mod_wc_goa, view = "HYCOM_temp", cond = list("Depth_fct2"))
